...
def build_graph(features, label, mode, params, config=None):
    ...
    if mode == "infer":
        indices = twml.limit_bits(
            features["input_sparse_tensor_indices"], num_bits)
        ...
        sparse_tf = tf.SparseTensor(
            ...
            values=get_feature_values(
                features["input_sparse_tensor_values"], params),
    ...
    if params.lolly_model_tsv:
        ...
        discretizer = TFModelDiscretizerBuilder(
            num_bits).build(tf_model_initializer)
    else:
        discretizer = hub.Module(params.discretizer_save_dir)
        bias_initializer, weight_initializer = None, None

    input_sparse = discretizer(
        sparse_tf, signature="hashing_discretizer_calibrator")
    ...
    if mode != "infer":
        ...
        if params.replicate_lolly:
            loss = tf.reduce_mean(
                tf.math.squared_difference(logits, lolly_activations))
        else:
...
def earlybird_output_fn(graph_output):
    export_outputs = {
        tf.saved_model.signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY: tf.estimator.export.PredictOutput(
            {"prediction": tf.identity(
                graph_output["output"], name="output_scores")}
        )
    }
...
if __name__ == "__main__":
    ...
    feature_config_module = all_configs.select_feature_config(
        opt.feature_config)
    ...
    parse_fn = twml.parsers.get_sparse_parse_fn(
        feature_config,
        keep_fields=("ids", "keys", "values",
                     "batch_size", "total_size", "codes"),
    )
