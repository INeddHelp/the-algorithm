...
def get_sparse_batch_supervised_input_receiver_fn(feature_config,
                                                  keep_fields=None):
    ...
    def supervised_input_receiver_fn():
        ...
        features = (bpr.get_sparse_features()
                    if keep_fields is None else bpr.get_features(keep_fields))
        features["weights"] = bpr.weights
        labels = bpr.labels
        features, labels = bpr.apply_filter(features, labels)

        return export.SupervisedInputReceiver(features, labels,
                                              receiver_tensors)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            output.update(
                twml.export_output_fns.batch_prediction_continuous_output_fn(
                    output)[tf.saved_model.signature_constants.
                            DEFAULT_SERVING_SIGNATURE_DEF_KEY].outputs)
        return output

    return new_build_graph_fn


def export_model_for_train_and_infer(trainer,
                                     feature_config,
                                     keep_fields,
                                     export_dir,
                                     as_text=False):
...
def export_all_models(
    ...
    if (trainer.params.get("distributed", False)
            and not trainer.estimator.config.is_chief):
        tf.logging.info(
            "Trainer.export_model ignored due to instance not being chief.")
        return
