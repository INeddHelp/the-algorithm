...
def load_initializers_from_csv(
    ...
    logging.info(
        "Loaded %d embeddings in %d seconds." % (
            len(vocab), time.time() - start)
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            raise ValueError(
                "Weight initializer should be a `Constant` or `None`.")
    ...
    def build(self, input_shapes):
        ...
        if self.use_placeholder:
            embedding_weight_initializer = (
                twml.contrib.initializers.PlaceholderInitializer(
                    shape, self.dtype)
            )
    ...
    def call(
        ...
        if oov_summaries:
            oov_count = tf.reduce_sum(
                tf.cast(tf.math.equal(input_ids, self.oov_word_id),
                        tf.dtypes.float32)
            )
            valid_count = tf.reduce_sum(
                tf.cast(tf.math.not_equal(
                    input_ids, PAD_WORD_ID), tf.dtypes.float32)
            )
        ...
        output_shape = inputs.shape.concatenate(
