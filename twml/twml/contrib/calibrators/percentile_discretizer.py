...
class PercentileDiscretizerFeature(CalibrationFeature):
    ...
    def _gather_debug_info(values, indices, bin_vals, bin_counts_buffer):
        ...
        nonempty_bins = np.append(np.flatnonzero(np.diff(bin_vals)),
                                  len(bin_vals) - 1)
    ...
    def calibrate(self,
        ...
        if values.size < bin_vals.size:
            ...
            percentiles_tarray = twml.Array(
                percentiles.reshape(percentiles.size, 1))
          ...
          caught an error (see previous stdout). Error code: """ % err)
            ...
            if bin_counts_buffer is not None:
                self._gather_debug_info(values, indices, bin_vals,
                                        bin_counts_buffer)
...
class PercentileDiscretizerCalibrator(Calibrator):
    ...
    def __init__(self,
        ...
        self._bin_vals = np.empty(
            0, dtype=np.float32)  # Note changed from 64 (v1) to 32 (v2)
    ...
    def accumulate_feature(self, output):
        ...
        """
        return self.accumulate(output["feature_ids"], output["feature_values"],
                               output["weights"])
    ...
    def accumulate(self, feature_keys, feature_vals, weights=None):
        ...
        if feature_keys.ndim != 1:
            raise ValueError("Expecting 1D feature_keys, got %dD" %
                             feature_keys.ndim)
        if feature_vals.ndim != 1:
            raise ValueError("Expecting 1D feature_values, got %dD" %
                             feature_vals.ndim)
        ...
        if weights is not None:
            ...
            if weights.ndim != 1:
                raise ValueError("Expecting 1D weights, got %dD" %
                                 weights.ndim)
        ...
        if weights is None:
            weights = np.full(feature_vals.size,
                              fill_value=DEFAULT_SAMPLE_WEIGHT)
        ...
        for feature_id in unique_keys:
            ...
            if feature_id not in self._discretizer_feature_dict:
                ...
                self._discretizer_feature_dict[
                    feature_id] = discretizer_feature
            else:
                discretizer_feature = self._discretizer_feature_dict[
                    feature_id]
            discretizer_feature.add_values({
                "values": feature_vals[idx],
                "weights": weights[idx]
            })
    ...
    def calibrate(self, debug=False):
        ...
        for i, feature_id in enumerate(discretizer_features_dict):
            ...
            discretizer_features_dict[feature_id].calibrate(
                self._bin_vals[
                    self._hash_map[feature_id]],  # Gets feature-values
                percentiles,
                percentile_indices,
            ...
            if (i % max(1.0, round(n_feature / 20))
                    == 0) or (i == n_feature - 1):
    ...
    def _create_discretizer_layer(self, n_feature, hash_map_keys,
    ...
    def to_layer(self, name=None):
        ...
        feature_offsets = np.arange(0,
                                    max_discretizer_feature,
                                    self._n_bin + 1,
                                    dtype="int64")
        hash_map_keys = np.array(list(self._hash_map.keys()), dtype=np.int64)
        hash_map_values = np.array(list(self._hash_map.values()),
                                   dtype=np.float32)
    ...
    def write_summary(self, writer, sess=None):
        ...
        histo = tf.summary.histogram("discretizer_feature_bin_counts",
                                     bin_counts_ph)
    ...
    def write_summary_json(self, save_dir, name="default"):
        ...
        discretizer_export_bin_filename = os.path.join(save_dir,
                                                       name + "_bin.json")
        ...
        twml.write_file(discretizer_export_bin_filename,
                        discretizer_export_bin_dict,
                        encode="json")
