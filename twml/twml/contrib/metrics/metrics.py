...
def get_partial_multi_binary_class_metric_fn(
    ...
    def get_eval_metric_ops(graph_output, labels, weights):
        if predcols is None:
            ...
            for col in predcol_list:
                if not 0 <= col < graph_output["output"].shape[class_dim]:
                    raise AssertionError("Invalid Prediction Column Index !")
            preds = tf.gather(
        ...
        metrics_op = get_multi_binary_class_metric_fn(
            metrics, classes, class_dim)
...
def mean_numeric_label_topK(labels, predictions, weights, name, topK_id):
    top_k_labels = tf.gather(
        params=labels, indices=topK_id, axis=0)  # [topK, 1]
...
def mean_gated_numeric_label_topK(labels, predictions, weights, name, topK_id, bar=2.0):
    if not (isinstance(bar, int) or isinstance(bar, float)):
        raise AssertionError("bar must be int or float")
    top_k_labels = tf.gather(
        params=labels, indices=topK_id, axis=0)  # [topK, 1]
...
DEFAULT_NUMERIC_METRICS = [
    "mean_numeric_label_topk", "mean_gated_numeric_label_topk"]
...
def get_metric_topK_fn_helper(
    ...
    def get_eval_metric_ops(graph_output, labels, weights):
        ...
        if predcol is None:
            ...
            pred = tf.reshape(
                graph_output["output"][:, predcol], shape=[-1, 1])
            labels = tf.reshape(labels[:, labelcol], shape=[-1, 1])
        numOut = graph_output["output"].shape[1]
        pred_score = tf.reshape(
            graph_output["output"][:, numOut - 1], shape=[-1, 1])
...
def get_single_binary_task_metric_fn(
    ...
    def get_eval_metric_ops(graph_output, labels, weights):
        ...
        if use_topK:
            ...
            metrics_numeric_res = metric_op_numeric(
                graph_output, labels, weights)
