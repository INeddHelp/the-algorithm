...
def ndcg(
    ...
    with tf.variable_scope(name, "ndcg", (labels, predictions)):
        ...
        predicted_scores = tf.to_float(
            predictions, name="predictions_to_float")
...
def get_search_metric_fn(
    binary_metrics=None, search_metrics=None, ndcg_top_ks=None, use_binary_metrics=False
):
    """
            ...
            if search_metric_factory:
                if metric_name == "ndcg":
                    for top_k in ndcg_top_ks:
                        ...
                        eval_metric_ops[metric_name_ndcg_top_k] = (
                            value_op, update_op)
            else:
                raise ValueError(
                    "Cannot find the search metric named " + metric_name)
            ...
            for metric_name in binary_metrics:
                ...
                if binary_metric_factory:
                    value_op, update_op = binary_metric_factory(
                        labels=labels,
                        predictions=(
                            hard_preds if requires_threshold else preds),
                        weights=weights,
                        name=metric_name,
                    )
