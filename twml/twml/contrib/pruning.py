...
def computational_cost(op_or_tensor, _observed=None):
    ...
    masks_extended = [
        mask.extended for mask in tf.get_collection(MASK_COLLECTION)]
    ...
    if op.op_def.name in ["Conv2D"]:
        ...
        inner_prod_cost = tf.reduce_prod(
            kernel_shape[:2]) * input_shape[-1] * 2.0 - 1.0
...
def update_pruning_signals(loss, decay=0.96, masks=None, method="Fisher"):
    ...
    with tf.variable_scope("pruning_opt", reuse=True):
        ...
        signals_batch = [tf.squeeze(
            tf.reduce_mean(tf.square(g), 0)) for g in grads]
            ...
            updates.append(
                tf.cond(
                    ...
                    lambda: tf.scatter_update(
                        masks[k],
                        tf.Print(
                            i, [i], message="Pruning layer [{0}] at index ".format(
                                k)
                        ),
                        0.0,
                    ),
