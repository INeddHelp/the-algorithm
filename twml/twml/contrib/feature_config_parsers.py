...
def _validate_config_dict_v1(config_dict):
    ...
    def malformed_error(msg):
        raise ValueError("twml.FeatureConfig: Malformed feature_spec. %s" %
                         msg)
    ...
    if doc["format"] == "exported":
        dict_keys = [
            "features", "labels", "weight", "tensors", "sparse_tensors"
        ]
...
def _validate_config_dict_v2(config_dict):
    ...
    def malformed_error(msg):
        raise ValueError(
            "twml.contrib.FeatureConfig: Malformed feature_spec. %s" % msg)
...
def _create_feature_config_v1(config_dict, data_spec_path):
    ...
    if config_dict["format"] == "exported":
        ...
        if config_dict["weight"]:
            weight_feature = list(
                config_dict["weight"].values())[0]["featureName"]
...
def _create_feature_config_v2(config_dict, data_spec_path):
    ...
    if config_dict["format"] == "exported":
        ...
        for sparse_group in config_dict["sparseFeatureGroups"]:
            ...
            fnames = [
                sparse_group["features"][fid]["featureName"] for fid in fids
            ]
        ...
        for dense_group in config_dict["denseFeatureGroups"]:
            ...
            fnames = [
                dense_group["features"][fid]["featureName"] for fid in fids
            ]
        ...
        for dense_features in config_dict["denseFeatures"]:
            ...
            fnames = [
                dense_features["features"][fid]["featureName"] for fid in fids
            ]
...
def create_feature_config(config_path, data_spec_path):
    ...
    if ext not in [".yaml", ".yml"]:
        raise ValueError(
            "create_feature_config_from_yaml: Only .yaml/.yml supported")
