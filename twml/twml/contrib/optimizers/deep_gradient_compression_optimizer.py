...
def get_top_row_indices(values, density):
    ...
    try:
        ...
        k = tf.maximum(
            tf.constant(1),
            tf.cast(tf.scalar_mul(density, tf.cast(row_num, tf.float32)),
                    tf.int32),
        )
...
class DeepGradientCompressionOptimizer(tf.train.GradientDescentOptimizer):
    ...
    def __init__(
        ...
        super(DeepGradientCompressionOptimizer,
        ...
        self._density_decay_steps_t = tf.convert_to_tensor(
            density_decay_steps, dtype)
        self._density_decay_rate_t = tf.convert_to_tensor(
            density_decay_rate, dtype)
    ...
    def _apply_dense(self, grad, var):
        if not self._accumulation:
            ...
            sparsified_grad = tf.IndexedSlices(sparsified_values,
                                               sparsified_indices)

            return super(DeepGradientCompressionOptimizer,
                         self)._apply_sparse_duplicate_indices(
                             sparsified_grad, var)
        ...
        update_var = super(DeepGradientCompressionOptimizer,
                           self)._apply_sparse_duplicate_indices(
                               sparsified_grad, var)

        update_g_buffer = tf.scatter_update(g_buffer, sparsified_indices,
                                            tf.zeros_like(sparsified_values))
