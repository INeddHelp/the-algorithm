...
def total_weight_metric(
    ...
    with tf.variable_scope(name, "total_weight", (labels, predictions, weights)):
        total_weight = _metric_variable(
            name="total_weight", shape=[], dtype=tf.float64)
...
def num_samples_metric(
    ...
    with tf.variable_scope(name, "num_samples", (labels, predictions, weights)):
        num_samples = _metric_variable(
            name="num_samples", shape=[], dtype=tf.float64)
...
def prediction_std_dev(
    ...
    with tf.variable_scope(name, "pred_std_dev", (labels, predictions, weights)):
        ...
        update_total_weighted_preds_sq = tf.assign_add(
            total_weighted_preds_sq, tf.reduce_sum(
                weights * predictions * predictions)
        )
        update_total_weights = tf.assign_add(
...
def _get_arce_predictions(
    ...
    if up_weight is False:
        ...
        label_comp = tf.subtract(tf.to_float(
            tf.size(labels)), tf.reduce_sum(labels))
    ...
    pred_num = tf.multiply(predictions, normalizer,
                           name="normalized_pred_numerator")
    pred_denom = tf.add(pred_num, pred_comp_norm,
                        name="normalized_pred_denominator")
...
def rce(
    ...
    with tf.variable_scope(name, "rce", (labels, predictions, weights)):
        ...
        total_loss = _metric_variable(
            name="total_loss", shape=[], dtype=tf.float32)
        total_weight = _metric_variable(
            name="total_weight", shape=[], dtype=tf.float32)
        ...
        logloss = _binary_cross_entropy(
            pred=clip_p, target=labels, name="logloss")

        logloss_weighted = tf.multiply(
            logloss, weights, name="weighted_logloss")
        ...
        baseline_ce = _binary_cross_entropy(
            pred=ctr1, target=ctr1, name="baseline_ce")
        pred_ce = tf.truediv(total_loss, total_weight, name="pred_ce")

        rce_t = tf.multiply(
            1.0 - tf.truediv(pred_ce, baseline_ce), 100, name="rce")

        # metric update subgraph
        ctr2 = tf.truediv(update_total_positive,
                          update_total_weight, name="ctr_update")
        # Note: we don't have to keep running averages for computing baseline CE. Because the prediction
        # is constant for every sample, we can simplify it to the formula below.
    ...
    with tf.variable_scope(name, "metric_std_err", (labels, predictions, weights)):
        ...
        if weights is None:
            weights = tf.ones_like(
                labels, dtype=tf.float64, name="default_weight")
        else:
            weights = tf.cast(weights, tf.float64)
        ...
        def compute_output(agg_1, agg_2, samp_cnt):
            ...
            # i.e. d M(agg1) / d agg1
            metric_prime = tf.gradients(
                metric_value, agg_1, stop_gradients=agg_1)
        ...
        metric_stderr = compute_output(
            aggregates_1, aggregates_2, sample_count)
...
def lolly_nrce(
    ...
    with tf.variable_scope(name, "lolly_nrce", (labels, predictions, weights)):
        ...
        positive_weights = tf.multiply(
            labels, weights, name="positive_weights")
        ...
        # metric value retrieval subgraph
        # ctr of this batch
        positive_rate = tf.truediv(
            total_positive, total_weight, name="positive_rate")
        # Note: we don't have to keep running averages for computing baseline CE. Because the prediction
        # is constant for every sample, we can simplify it to the formula below.
        ...
        normalizer = tf.truediv(
            total_positive, total_prediction, name="normalizer")
...
SUPPORTED_BINARY_CLASS_METRICS = {
    ...
    "false_positives_at_percentiles": (
        partial(tf.metrics.false_positives_at_thresholds,
                thresholds=PERCENTILES),
        False,
    ),
    "false_negatives_at_percentiles": (
        partial(tf.metrics.false_negatives_at_thresholds,
                thresholds=PERCENTILES),
        False,
    ),
    "true_positives_at_percentiles": (
        partial(tf.metrics.true_positives_at_thresholds,
                thresholds=PERCENTILES),
        False,
    ),
    "true_negatives_at_percentiles": (
        partial(tf.metrics.true_negatives_at_thresholds,
                thresholds=PERCENTILES),
        False,
    ),
    # tensorflow metrics
    "roc_auc": (
        partial(tf.metrics.auc, curve="ROC",
                summation_method="careful_interpolation"),
        False,
    ),
    "pr_auc": (
        partial(tf.metrics.auc, curve="PR",
                summation_method="careful_interpolation"),
        False,
    ),
    # tensorboard curves
    ...
    "deprecated_arce_original": (
        partial(rce, normalize=True, arce=True,
                up_weight=False, deprecated_rce=True),
        False,
    ),
}
    ...
    def get_eval_metric_ops(graph_output, labels, weights):
        ...
        for metric in metrics:
            if isinstance(metric, tuple) and len(metric) == 3:
                ...
                # metric name are case insensitive.
    ...
    def get_eval_metric_ops(graph_output, labels, weights):
        ...
        for i in range(num_labels):
            ...
            for metric in metrics:
                ...
                class_metric_name = (
                    metric_name + "_" +
                    (classes[i] if classes is not None else str(i))
                )
                ...
                class_hard_preds = tf.gather(
                    hard_preds, indices=[i], axis=class_dim)

                if num_weights is None:
                    class_weights = None
                elif num_weights == num_labels:
                    class_weights = tf.gather(
                        weights, indices=[i], axis=class_dim)
                elif num_weights == 1:
                ...
                if metric_factory:
                    ...
                    raise ValueError(
                        "Cannot find the metric named " + metric_name)
...
def _get_uncalibrated_metric_fn(calibrated_metric_fn, keep_weight=True):
    ...
    def get_eval_metric_ops(graph_output, labels, weights):
        ...
        with tf.variable_scope(metric_scope):
            ...
            un_calibrated_weights = weights if keep_weight else tf.ones_like(
                weights)
            uncalibrated_output = {
