...
class Trainer(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._using_hogwild = True if os.environ.get(
            "TWML_HOGWILD_PORTS") else False
        ...
        if init_from_dir is not None and init_map is None:
            raise ValueError(
                "Need to provide init_map when init_from_dir is provided.")
    ...
    def _hogwild_setup(self):
        ...
        if ports:
            ...
            if self._num_workers + 1 != len(self._ports):
                raise ValueError(
                    "Number of (workers + PS) and ports need to match")
        else:
            if self._num_workers > 1:
                raise ValueError(
        ...
        num_threads_per_worker = int(
            math.ceil(float(num_threads) / self._num_workers))
    ...
    def check_params(self):
        ...
        if "train_batch_size" in param_values:
            if not isinstance(self._params.train_batch_size, int):
                raise ValueError(
                    "Expecting params.train_batch_size to be an integer.")
        ...
        if "eval_batch_size" in param_values:
            if not isinstance(self._params.eval_batch_size, int):
                raise ValueError(
                    "Expecting params.eval_batch_size to be an integer.")
    ...
    def export_model_effects(self, export_path, feature_spec=None, log_features=True):
        ...
        if feature_spec:
            if log_features:
                ...
                try:
                    model_hash = self.experiment_tracker.compute_model_hash(
                        export_path)
                    metrics.log_usage(
                        "dbv2",
            ...
            twml.contrib.export.export_fn.export_feature_spec(
                export_path, feature_spec)
    ...
    def best_or_latest_checkpoint(self):
        if self._is_early_stopping:
            ...
            raise ValueError("Best checkpoint not found at %s." %
    ...
    def learn(
        ...
        if train_max_steps is not None and train_max_steps > 0:
            ...
            stop_at_step_hook = twml.hooks.StopAtStepHook(
                last_step=train_max_steps)
    ...
    def train_and_evaluate(
        ...
        if early_stop_patience > 0:
            ...
            train_early_stop_hook = twml.hooks.StopIfExistsHook(
                early_stop_path)
    ...
    def start_tensorboard(self, port=None):
        ...
        if port is None:
            if "tensorboard_port" not in self.params.values():
                raise ValueError(
                    "You must specify a port for tensorboard to run on.")
