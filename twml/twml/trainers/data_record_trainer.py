...
class DataRecordTrainer(Trainer):  # pylint: disable=abstract-method
        ...
        if eval_name == "train":
            ...
            if not self.params.get("allow_train_eval_overlap"):
                ...
                if self._train_files and self._eval_files:
                    overlap_files = set(self._train_files) & set(
                        self._eval_files)
                else:
                    overlap_files = set()
                if overlap_files:
    ...
    def add_parser_arguments():
        ...
        """
        parser = super(DataRecordTrainer,
                       DataRecordTrainer).add_parser_arguments()
        parser.add_argument(
            "--train.files_list",
            "--train_files_list",
            type=str,
    ...
    def contrib_run_feature_importances(
        ...
        if algorithm == TREE:
            ...
            logging.info(
                "Using the tree algorithm with kwargs {}".format(kwargs))
    ...
    def export_model(
        ...
        if isinstance(serving_input_receiver_fn, twml.feature_config.FeatureConfig):
            ...
            raise ValueError(
                "Expecting Function for serving_input_receiver_fn")
    ...
    def get_train_input_fn(
        ...
        **kwargs,
        ...
        if log_tf_data_summaries and not initializable:
            raise ValueError(
                "Require `initializable` if `log_tf_data_summaries`.")
        ...
        return lambda: twml.input_fns.default_input_fn(
            ...
            shuffle=shuffle,
            shuffle_files=(
                shuffle if shuffle_files is None else shuffle_files),
    ...
    def evaluate(self, input_fn=None, steps=None, hooks=None, name=None):
        ...
        input_fn = input_fn if input_fn else self.get_eval_input_fn(
            repeat=False)
