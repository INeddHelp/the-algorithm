...
class StepProgressHook(tf.train.SessionRunHook):
    ...
    def begin(self):
        ...
        if self._global_step_tensor is None:
            raise RuntimeError(
                "Global step should be created to use StepProgressHook.")
...
class EarlyStopHook(GetMetricsHook):
    ...
    def early_stop(self, epoch):
        ...
        if self._is_better_than(current_metric, self._best_metric):
            ...
            try:
                eval_checkpoint = tf.train.latest_checkpoint(
    ...
    def begin(self):
        ...
        self._latest_checkpoint_path = tf.train.latest_checkpoint(
