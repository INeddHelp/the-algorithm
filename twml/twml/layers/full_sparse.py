...
class FullSparse(Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(FullSparse, self).__init__(
    ...
    def build(self, input_shapes):
        ...
        if isinstance(input_shapes, (list, tuple)):
            ...
            if not is_compatible:
                raise ValueError(
                    "Input shapes %s are not compatible." % input_shapes)
        else:
        ...
        (split_dim, other_dim) = (
            (in_shape, out_shape) if self.partition_axis == 0 else (
                out_shape, in_shape)
        )
        requested_size = (
            math.ceil(float(split_dim) / num_partitions) *
            other_dim * dtype.size
        )
