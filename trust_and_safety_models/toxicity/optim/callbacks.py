        ...
        with writer.as_default():
            with tf.GradientTape() as tape:
                ...
                loss = self.model.compiled_loss(
                    y_true=self.y_batch, y_pred=y_pred)
                gradient_norm = tf.linalg.global_norm(
                    tape.gradient(loss, self.model.trainable_weights)
                )

            tf.summary.scalar(
                "gradient_norm", data=gradient_norm, step=self.counter)
        writer.flush()
...
class AdditionalResultLogger(tf.keras.callbacks.Callback):
        ...
        **kwargs,
        ...
        try:
            ...
            if 1 < fixed_recall <= 100:
                ...
                raise ValueError(
                    "Threshold should be between 0 and 1, or 0 and 100")
    ...
    def additional_evaluations(self, step, eval_time):
        ...
        for k, v in metrics.items():
            self.best_metrics[f"max_{k}"] = max(
                v, self.best_metrics[f"max_{k}"])
