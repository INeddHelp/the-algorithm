...
print(train_query)
train = cq_loader.load_features(
    ptos_prototype, "", "", custom_query=train_query)
...
def get_positive_weights():
    ...
    pos_weight_tensor = tf.cast(
        label_weights_df.sort_values(
            by="label").positive_class_weight, dtype=tf.float32
    )
...
test_only_nsfw = test.filter(
    lambda x, y: tf.greater_equal(x["precision_nsfw"], 0.95))
...
def eval_model(model, df):
    with mirrored_strategy.scope():
        targets = np.stack(
            list(df.map(lambda x, y: y).as_numpy_iterator()), axis=0)
